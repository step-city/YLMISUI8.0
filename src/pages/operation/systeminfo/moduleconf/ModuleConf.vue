...
<div slot="content">
         <yl-toolbar>
              <el-form  :inline="true"  >
                            ...
                            <el-form-item class="form-content-vertical">
                               <el-button type="primary" size="small" @click="_saveData" :loading="loading">保存</el-button>   
                            </el-form-item>
                 </el-form>    
          </yl-toolbar>
          <el-form  :model="formModel"  :rules="rules"  ref="formModel" label-width="100px" >
                <el-tabs  type="border-card" active-name="baseInfoConf" >
                    <el-tab-pane label="基本信息配置" name="baseInfoConf" > 
                        ...
                        <el-tab-pane label="输出信息配置" name="extensionOne"> 
                            <el-form-item label="模板设计器" >
                                <el-button type="primary" icon="edit" @click="print_design">调用模板设计器</el-button>
                            </el-form-item>
                            <el-form-item label="输出信息配置" prop="extensionOne">
                                <yl-jsoneditor ref="extensionOne" v-model="formModel.extensionOne" style="height:600px"></yl-jsoneditor>
                            </el-form-item>
                        </el-tab-pane>
                </el-tabs>
            </el-form>
...
export default {
    ...
    methods:{
        print_design() {
                ...
                let LODOP=getLodop();
                //调用配置
                let outPutConf=JSON.parse(this.formModel.extensionOne,util.dealFunction).outPutConf;
                let printConf=outPutConf.printConf;
                if(printConf.InterceptEvent.initTemplate!==undefined){
                    if(printConf.InterceptEvent.initTemplate.isOn){
                        printConf.InterceptEvent.initTemplate.event(printConf.content,outPutConf.displayModel);
                    }
                }
                if(printConf.eventConf!=undefined){
                            if(printConf.eventConf.isOn){
                                printConf.eventConf.event(printConf.content,_this);
                            }
                }
                LODOP.PRINT_DESIGN();
        },
                ...
                if(this.formModel.id===""){
                    ...
                    requestGetModuleConfForEdit(inputArr).then((data)=>{
                            if(data.success){
                                if(data.result.moduleConf!=null){
                                    //提交重复
                                    this.formModel.id=data.result.moduleConf.id;
                                    }
                                //提交数据    
                                this._onSubmit();
                            }
