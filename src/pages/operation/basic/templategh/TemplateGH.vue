...
<div slot="content">
  <yl-columnlay>
              <div slot="left"  >
                    <yl-panelpage  isZone :helpIsShow="false" :treeLoading="treeLoading" :reloadIsShow="false" :titleName="'工号模板树'" :icon="'icon-tree'" @reload="_treeReload" :widthNum="'210px'" >
                        <div slot="content">
                ...
                <div slot="right" >
                    <yl-layout>
                        ...
                        <yl-toolbar>
                            <el-button-group>
                                    <el-button type="primary" v-permissionSetting="'Pages.Operation.Data.TemplateGHs.Create'"   icon="plus" size="small"  @click="_add" >添加</el-button>
                                    <el-button type="primary" v-permissionSetting="'Pages.Operation.Data.TemplateGHs.Edit'"   icon="edit"  size="small" :disabled="selectRows.length===0" @click="_edit">编辑</el-button>
                                    <el-button type="danger"  v-permissionSetting="'Pages.Operation.Data.TemplateGHs.Delete'"   icon="delete" size="small"  :disabled="selectRows.length===0" @click="_delete" >删除</el-button>
                             </el-button-group>
                        </yl-toolbar>
                        </div>
    ...
    methods:{
        ...
        _edit(){
            if(this.selectNode.id!=this.guidOfNull()){
                ...
                this.$message.error('第一层不能修改');
            }
        },
        _delete(){
            var params={}; 
            params.node=this.selectRows[0].id;
            requestGetTemplateGHByParent(params).then(data=>{
            ...
            this.$confirm('确认要删除所选项目吗?', '提示', {
                ...
                requestDeleteTemplateGH(obj).then(data =>{
                    ...
                    this.$message.error('第一层不能删除！');
                }
                }else{
                    this.$message.error('还有未删除的子节点！');
                }
            })
