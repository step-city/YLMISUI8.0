  ...
  computed: {
   ...
   methods:{
       _changeText(a,b){
        ...
        _getModuleConf(){
           let inputArrs=[ 
                    {dataField:"ExtensionTwo",op:"EQ",dataValue:'table32Order'},
                  ];
            fetch({ 
            ...
            this.$emit('getDbData');
             ...
             if(this.formModel.order.id===''){
                 ...
                 this.jinZ=this.dbData.Num-kd;
                     ...
                     //不陈皮
                     if(kd===0){
                        kd=Math.abs(this.dbData.Num-this.formModel.order.d_Col37)*this.formModel.order.d_Col39/100
                    }
                     this.jinZ=Math.abs(this.dbData.Num-this.formModel.order.d_Col37)-kd;
                 }
             }else{
                  if(kd===0){
        ...
        async _getDataPageList(){
          ...
          _uploadData(dataModel){ 
            let _this=this;
                ...
                let cCBeforeEvent=_this.gbGlobConf.ioConf.InterceptEventEvent.cCBeforeUplodEvent;
                let cCAfterEvent=_this.gbGlobConf.ioConf.InterceptEventEvent.cCAfterUplodEvent;
        ...
        async  _importWight(){
            ...
            if(!_this.formModel.order.b_Col28){
             ...
             if(result===null){
                        ...
                        if(_this.formModel.order.b_Col27){
       ...
       _unCP(){
            ...
            if(this.formModel.order.v_Col11!==""){
                if(this.formModel.order.b_Col28){
                            ...
                            if(!carObj[0].b_Col17){
                                //不称皮重
                                this.formModel.order.d_Col37=carObj[0].d_Col20;
                            }
                    }
            }else{
                ...
                if(dataModel.order!==null){
                   //进场未出场
                        if(dataModel.order.b_Col27){
                            ...
                            dataModel.order.b_Col28=this.formModel.order.b_Col28;
             ...
             if(this.formModel.order.v_Col11!==""){
                   //目前正在作业，请稍后刷卡！
                    this.$message.warning('目前正在作业，请稍后刷卡！');
                     ...
                     //空闲状态 填充数据
                     //构造formModel
                     if(this.currentView==='commonGb_CC'){
                         if(this.carNumConf.dataList.length===0){
                             //加载数据
                             this.carNumConf.dataList= await  this.asyncFunc({
                                                    url:this.apiList.carNumParams,
                                                    method:'post',
                                                    data:{
                                                        queryConditionItem:[
                             ...
                             this.bakUpData.carNumConf=util.deepCopy(this.carNumConf); 
                               //获取材料列表信息
                              this.materialConf.dataList= await  this.asyncFunc({
                                                            url:this.apiList.commonMaterialParams,
                                                            method:'post',
                                                            data:{
                                                                queryConditionItem:[
                                                                                    {dataField:"OrgId",op:'EQ',dataValue:this.getUserInfo().user.manageOrgId,DataFieldTwo:"",TwoFieldRelation:""}
                             ...
                             this.bakUpData.materialConf=util.deepCopy(this.materialConf);
                         }
                        let model={order:{}};
                        model.order.id='';
                        ...
                        model.order.b_Col28=false;
                        //处理ic卡出场问题
                         let carObj=this.bakUpData.carNumConf.dataList.filter(i=>{
                                return i.v_Col6===model.order.v_Col11;
                            })
                            if(!carObj[0].b_Col17){
                                //不称皮重
                                model.order.b_Col28=true;
                                model.order.d_Col37=carObj[0].d_Col20;
                            }else{
                                //称皮重
                                model.order.b_Col28=false;
                            }
